...
function auditLogDelete(inputPath, tokenId, gpid) {
    ...
    // console.log('## check input ',parsedInput);
    // console.log('## input medias ',parsedInput.medias);
    // console.log('## input global annotation ', parsedInput.globalannotations);
    // console.log('## input local annotation ', parsedInput.localannotations);
    ...
    // parsedInput.medias.forEach((item) => {
    //     let fileName = item.filepath.split('/');
    //     let fileNameStr = fileName[fileName.length - 1].split('.')[0] + ' for ' + parsedInput.gpid; 
    //     callAuditLogForDelete(fileNameStr, tokenId);
    // });
    // parsedInput.globalannotations.forEach((item) => {
    //     let fileName = item.filepath.split('/');
    //     let fileNameStr = fileName[fileName.length - 1].split('.')[0] + ' for ' + parsedInput.gpid;
    //     console.log('### filename string ', fileNameStr);
    //     callAuditLogForDelete(fileNameStr, tokenId);
    // });
    // parsedInput.localannotations.forEach((item) => {
    //     let fileName = item.filepath.split('/');
    //     let fileNameStr = fileName[fileName.length - 1].split('.')[0] + ' for ' + parsedInput.gpid; 
    //     callAuditLogForDelete(fileNameStr, tokenId);
    // });
